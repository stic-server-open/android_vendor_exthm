...
function write_blueprint_header() {
    ...
    if [[ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]]; then
        ...
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The exTHmUI Project\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            ...
            printf "# Copyright (C) 2017 The exTHmUI Project\n" >> $1
            ...
            printf "# Copyright (C) 2017-$YEAR The exTHmUI Project\n" >> $1
        else
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$AOSP_ROOT"/vendor/exthm/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
