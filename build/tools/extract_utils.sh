...
function setup_vendor() {
    ...
    export EXTHM_ROOT="$3"
    if [ ! -d "$EXTHM_ROOT" ]; then
        echo "\$EXTHM_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$EXTHM_ROOT/$OUTDIR" ]; then
        mkdir -p "$EXTHM_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$EXTHM_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$EXTHM_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$EXTHM_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$EXTHM_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        if [ $YEAR -eq 2019 ]; then
            printf "# Copyright (C) 2019 The exTHmUI Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The exTHmUI Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
            printf "# Copyright (C) 2019-$YEAR The exTHmUI Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The exTHmUI Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The exTHmUI Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$EXTHM_ROOT"/prebuilts/tools-lineage/common/smali/baksmali.jar
        export SMALIJAR="$EXTHM_ROOT"/prebuilts/tools-lineage/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$EXTHM_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$EXTHM_ROOT"/prebuilts/tools-lineage/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$EXTHM_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$EXTHM_TARGET" >/dev/null; then
...
function extract() {
    ...
    local OUTPUT_ROOT="$EXTHM_ROOT"/"$OUTDIR"/proprietary
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$EXTHM_ROOT"/vendor/exthm/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$EXTHM_ROOT"/"$OUTDIR"/radio
